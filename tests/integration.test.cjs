...
const makeProject = (loaderSource) => {
  const workspaceRoot = fs.mkdtempSync(
    path.join(os.tmpdir(), "sle-integration-"),
  );
  ...
  fs.writeFileSync(
    path.join(gameDir, "stateLayout.ts"),
    "export const stateLayoutDerived = {};\n",
    "utf8",
  );
  fs.writeFileSync(
    path.join(gameDir, "stateApp.ts"),
    "export const stateApp = {};\n",
    "utf8",
  );
  fs.writeFileSync(
    path.join(gameDir, "context.ts"),
    "import './layoutOverrides';\n",
    "utf8",
  );
  fs.writeFileSync(
    path.join(gameDir, "layoutOverrides.data.ts"),
    "export const layoutOverridesData = {};\n",
    "utf8",
  );
  fs.writeFileSync(
    path.join(gameDir, "layoutOverrides.ts"),
    loaderSource,
    "utf8",
  );
  return {
...
test("v11 analyzer and installer safely upgrade the standard v5 game-scale loader", (t) => {
  ...
  t.after(() =>
    fs.rmSync(project.workspaceRoot, { recursive: true, force: true }),
  );

  assert.equal(EXPECTED_BRIDGE_VERSION, 11);
  assert.equal(EXPECTED_BRIDGE_REVISION, "2026-08-28-canvas-preinit-guard-v1");
  ...
  assert.match(
    bundledRuntime,
    new RegExp(
      `LAYOUT_EDITOR_BRIDGE_REVISION\\s*=\\s*['"]${EXPECTED_BRIDGE_REVISION}['"]`,
    ),
  );
  const before = analyzeIntegration(project);
  assert.equal(
    before.checks.find((check) => check.id === "game-layout")?.status,
    "installable",
  );
  ...
  assert.equal(
    result.results.find((entry) => entry.id === "game-layout")?.action,
    "patched",
  );
  assert.equal(
    result.results.find((entry) => entry.id === "performance")?.action,
    "created",
  );
  assert.equal(
    result.results.find((entry) => entry.id === "test-book-request")?.action,
    "created",
  );
  assert.equal(
  ...
  assert.equal(
    fs.readFileSync(`${project.loaderPath}.sle-backup`, "utf8"),
    original,
  );

  const after = analyzeIntegration(project);
  assert.equal(
    after.checks.find((check) => check.id === "game-layout")?.status,
    "ok",
  );
  assert.equal(
    after.checks.find((check) => check.id === "performance")?.status,
    "ok",
  );
});
...
test("analyzer reports stale editor-owned bridge and built runtime files", (t) => {
  ...
  assert.equal(
    analysis.checks.find((check) => check.id === "runtime")?.status,
    "outdated",
  );
  assert.match(
    analysis.checks.find((check) => check.id === "runtime")?.note ?? "",
    /does not match/i,
  );
  assert.equal(
    analysis.checks.find((check) => check.id === "bridge")?.status,
    "outdated",
  );
  assert.match(
    analysis.checks.find((check) => check.id === "bridge")?.note ?? "",
    /does not match/i,
  );
  assert.equal(
    analysis.checks.find((check) => check.id === "dist")?.status,
    "outdated",
  );
  assert.match(
    analysis.checks.find((check) => check.id === "dist")?.note ?? "",
    /build/i,
  );
});
...
`;
  ...
  assert.equal(result.ok, true);
  assert.equal(
    result.results.find((entry) => entry.id === "app-import")?.action,
    "patched",
  );
  ...
  assert.ok(
    multilineEnd >= 0 && multilineEnd < sideEffect,
    "side-effect import was inserted inside the multiline import",
  );
  assert.ok(
    sideEffect < moduleBody,
    "side-effect import should remain in the import block",
  );
});
...
`,
  ...
  const before = analyzeIntegration(project);
  assert.equal(
    before.checks.find((check) => check.id === "utils-hook")?.status,
    "installable",
  );
  const result = installIntegration(project);
  assert.equal(
    result.results.find((entry) => entry.id === "utils-hook")?.action,
    "patched",
  );
