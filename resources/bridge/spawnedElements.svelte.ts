...
registerSpineLayoutBoundsResolver((node: AnyNode) => {
  const SetupPoseBoundsProvider = (SPINE_PIXI as AnyNode)
...
type WireOptions = {
  ...
  resolveSpriteInteraction?: (
    def: SpawnedElementDef,
  ) => SpawnedSpriteInteraction | undefined;
};
...
const isHoverChild = (def: SpawnedElementDef) =>
  def.kind === "sprite" && /hover/i.test(def.id);
...
const alignHoverToOwner = (hover: AnyNode, owner: AnyNode | undefined) => {
  ...
  hover.x =
    centre.x - (0.5 - (hover.anchor?.x ?? 0)) * width * (hover.scale?.x ?? 1);
  hover.y =
    centre.y - (0.5 - (hover.anchor?.y ?? 0)) * height * (hover.scale?.y ?? 1);
};
...
const resolveSpineData = (
  assetKey?: string,
): SPINE_PIXI.SkeletonData | null => {
  if (!assetKey || !options) return null;
  const asset = options.getLoadedAssets()?.[assetKey] as AnyNode;
  return asset?.bones && asset?.slots
    ? (asset as SPINE_PIXI.SkeletonData)
    : null;
};
...
const syncSiblingHover = (
  node: AnyNode,
  def: SpawnedElementDef,
  defs: SpawnedElementDef[],
) => {
  if (isHoverChild(def)) {
  ...
  const hoverChildren = defs
    .filter(
      (candidate) =>
        candidate.parentId === def.parentId && isHoverChild(candidate),
    )
...
const syncSpriteInteraction = (node: AnyNode, def: SpawnedElementDef) => {
  ...
  const usable =
    hitArea.width > 0 &&
    hitArea.height > 0 &&
    [hitArea.x, hitArea.y, hitArea.width, hitArea.height].every(
      Number.isFinite,
    );
  ...
  node.on("pointerup", (event: AnyNode) => {
    ...
    const current = node.__sleSpriteInteraction as
      | SpawnedSpriteInteraction
      | undefined;
    release();
...
const resolveParent = (
  ...
  const candidate =
    getRegisteredLayoutNodes().find(
      (node: AnyNode) => ensureLayoutId(node) === parentId,
    ) ?? null;
...
const isNodeShown = (node: AnyNode) => {
  let current = node;
  while (current) {
    if (!current.visible || current.renderable === false || current.alpha === 0)
      return false;
    current = current.parent;
  }
  return true;
...
const updateSiblingHoverFromPointer = (event: PointerEvent) => {
  ...
  // `Application.canvas` is a getter over `this.renderer.canvas`, so reading it
  // throws while the app exists but `init()` has not resolved yet - e.g. the game
  // is parked on an auth error modal and never boots Pixi. Go through the
  // renderer so an early pointer move cannot raise an uncaught TypeError.
  ...
  const x =
    ...
    ((screen?.width ?? canvasRect.width) / canvasRect.width);
  const y =
    (event.clientY - canvasRect.top) *
    ((screen?.height ?? canvasRect.height) / canvasRect.height);
  const insideCanvas =
    event.clientX >= canvasRect.left &&
  ...
  try {
    ...
    for (const def of defs) {
      ...
      if (def.kind === "container") {
        ...
        node = new PIXI.Sprite(
          resolveTexture(def.assetKey) ?? PIXI.Texture.EMPTY,
        );
      }
      node.label = def.id;
    ...
    for (const def of ordered) {
      ...
      if (isHoverChild(def)) {
        ...
        const hovered =
          renderable !== false && !!ownerNode?.__sleSiblingHovered;
