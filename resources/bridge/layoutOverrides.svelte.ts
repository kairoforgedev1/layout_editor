...
export const LAYOUT_EDITOR_BRIDGE_REVISION =
  "2026-08-28-canvas-preinit-guard-v1";
...
export type LayoutOverridesData = {
  version: number;
  profiles: Partial<
    Record<LayoutProfileName, Record<string, LayoutOverrideProps>>
  >;
...
export function compactLayoutOverrideEntry(
  source: LayoutOverrideProps,
): LayoutOverrideProps {
...
export function getGameLayout(): GameLayoutFrame | null {
  ...
  try {
    ...
    if (
      !value ||
      ![value.x, value.y, value.width, value.height, value.scale].every(
        Number.isFinite,
      ) ||
      value.width <= 0 ||
      value.height <= 0 ||
...
export function registerLayoutNode(node: AnyNode) {
  if (!node) return;
  const needsPostMountLayoutRefresh =
...
const isTextLike = (node: AnyNode) =>
  typeof node?.text === "string" && !!node?.style;
...
export function isTemporaryLayoutContainerNode(node: AnyNode): boolean {
  ...
  if (node.texture || isTextLike(node) || node.context || node.skeleton)
    return false;
...
const LAZY_FIELDS: LayoutOverrideField[] = [
  "width",
  "height",
  "fontSize",
  "align",
...
export function getAuthoredValues(
  node: AnyNode,
): Partial<Record<LayoutOverrideField, unknown>> {
...
export function refreshAuthoredDisplaySize(node: AnyNode) {
  const authored: Partial<Record<LayoutOverrideField, unknown>> | undefined =
    node?.__sleAuthored;
  ...
  try {
    ...
    authored.width =
      bounds.width *
      (Number.isFinite(authoredScaleX) ? Math.abs(authoredScaleX) : 1);
    authored.height =
      bounds.height *
      (Number.isFinite(authoredScaleY) ? Math.abs(authoredScaleY) : 1);
  } catch {
...
const visualMetrics = (
  node: AnyNode,
  scaleOverride?: { x: number; y: number },
): VisualMetrics => {
  try {
    ...
    if (validRect(bounds) && matrix) {
      ...
      return {
        ...
        origin: {
          x:
            right > left
              ? ((node.x ?? matrix.tx) - left) / (right - left)
              : (node.anchor?.x ?? 0),
          y:
            bottom > top
              ? ((node.y ?? matrix.ty) - top) / (bottom - top)
              : (node.anchor?.y ?? 0),
        },
      };
    }
  ...
  const width =
    (node.skeleton ? (fixedSpineSize?.width ?? 0) : Math.abs(node.width ?? 0)) *
    (currentScaleX > 1e-9 ? targetScaleX / currentScaleX : 1);
  const height =
    (node.skeleton
      ? (fixedSpineSize?.height ?? 0)
      : Math.abs(node.height ?? 0)) *
...
const localFrameOriginOffset = (node: AnyNode) => {
  try {
    ...
    if (matrix)
      return {
        x: (node.x ?? matrix.tx) - matrix.tx,
        y: (node.y ?? matrix.ty) - matrix.ty,
      };
  } catch {
...
const naturalSize = (node: AnyNode) => {
  ...
  return {
    w: Math.abs(node.width || 0) / sx,
    h: Math.abs(node.height || 0) / sy,
  };
};
...
export function getResponsiveConfig(
  id: string,
): ResponsiveConfig | false | undefined {
  return getMergedOverride(id).responsive;
}

const validRect = (
  rect?: Partial<ResponsiveRect> | null,
): rect is ResponsiveRect =>
  !!rect &&
...
let resolveSpineLayoutBounds:
  | ((node: AnyNode) => ResponsiveRect | null)
  | null = null;
...
export function getLayoutLocalBounds(node: AnyNode): ResponsiveRect | null {
  ...
  if (node.skeleton) {
    ...
    if (
      cached &&
      cached.skeletonData === skeletonData &&
      cached.boundsProvider === boundsProvider
    ) {
    ...
    fixedSpineLocalBounds.set(node, {
      skeletonData,
      boundsProvider,
      bounds: fixed,
    });
...
export function getLayoutDisplaySize(
  node: AnyNode,
...
export function getReferenceRect(
  ...
  const cfg: ResponsiveConfig =
    typeof config === "string" ? { ref: config } : config || {};
...
export function computeResponsive(
  ...
  const stretchOrigin =
    !containerLike && (cfg.stretchX || cfg.stretchY)
  ...
  if (cfg.aspect && !containerLike && !textLike) {
    ...
    if (nat.w > 0 && nat.h > 0) {
      ...
      out.scaleX =
        s * (cfg.aspectSign?.x ?? (Math.sign(node.scale?.x ?? 1) || 1));
      out.scaleY =
        s * (cfg.aspectSign?.y ?? (Math.sign(node.scale?.y ?? 1) || 1));
      const aspectOrigin = visualOriginRatio(node, {
        x: out.scaleX,
        y: out.scaleY,
      });
      ...
      if (cfg.x) {
        ...
        out.x =
          ref.x +
          align * (ref.width - displayW) +
          aspectOrigin.x * displayW +
      ...
      if (cfg.y) {
        ...
        out.y =
          ref.y +
          align * (ref.height - displayH) +
          aspectOrigin.y * displayH +
  ...
  // X axis
  if (cfg.stretchX) {
    const m0 = responsiveLengthToLocal(
      node,
      cfg,
      ref,
      "x",
    ...
    const m1 = responsiveLengthToLocal(
      node,
      cfg,
      ref,
      "x",
    ...
    out.x =
      getResponsiveAxisBase(node, cfg, ref, "x", cfg.x.anchor ?? 0) +
      responsiveLengthToLocal(node, cfg, ref, "x", cfg.x.offset ?? 0);
  }
  ...
  if (cfg.stretchY) {
    ...
    out.y =
      getResponsiveAxisBase(node, cfg, ref, "y", cfg.y.anchor ?? 0) +
      responsiveLengthToLocal(node, cfg, ref, "y", cfg.y.offset ?? 0);
  }
  ...
  if (containerLike) {
    ...
    if (lw != null || lh != null) {
      ...
      const height =
        lh != null ? lh / Math.abs(ownScaleY) : localBounds?.height;
...
export function applyLayoutOverrides(node: AnyNode, propKeys?: string[]) {
  ...
  if (isTemporaryLayoutContainerNode(node)) {
    ...
    if (authored && propKeys) {
      for (const field of FIELD_ORDER) {
        ...
        const coupledDimension =
          !isContainerLike(node) &&
          (((field === "width" || field === "height") &&
            propKeys.includes("scale")) ||
  ...
  let authored: Partial<Record<LayoutOverrideField, unknown>> =
    node.__sleAuthored;
  if (!authored) {
  ...
  if (
    textureNode &&
    (!node.__sleHasAuthoredTexture || authoredTextureChanged)
  ) {
  ...
  const responsiveConfig = responsiveEnabled(merged.responsive)
    ? merged.responsive
    : undefined;
  ...
  if (isTextLike(node)) {
    ...
    if (sample !== undefined) {
      if (node.__sleSampleWasSet === undefined)
        node.__sleSampleWasSet = node.text;
      node.text = sample;
