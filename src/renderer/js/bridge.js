...
export const EXPECTED_BRIDGE_REVISION = "2026-08-28-canvas-preinit-guard-v1";
...
export function syncGamePlayoutMode(mode = state.mode) {
  ...
  const supported = Object.values(GAME_PLAYOUT_EVENT_BY_MODE).every(
    (eventName) => state.gameEvents.includes(eventName),
  );
  if (name && supported) {
...
export function bridgeRequest(type, payload = {}, timeoutMs = 10000) {
  return new Promise((resolve, reject) => {
    ...
    entry.timer = setTimeout(() => {
      if (pendingRequests.delete(reqId))
...
export async function flushBridgeEdits(timeoutMs = 2500) {
  ...
  if (result?.ok === false)
    throw new Error(result.error || "Preview navigation changed.");
  return result;
}
...
export const currentBridgeNavigationSession = () =>
  awaitingSession ?? navigationSession;
