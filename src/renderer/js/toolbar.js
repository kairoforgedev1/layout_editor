...
import { openProject, startPreview, restartPreview, ensureRgs, saveOverrides } from './project.js';
import {
	undo,
	redo,
	resetElement,
...
import { setPreset, setResolution, toggleOrientation, stepPreset } from './viewport.js';
...
export async function doSave() {
	...
	try {
		...
		toast(`Cannot save yet: the game has not confirmed its latest layout edit (${error.message ?? error}).`, 'error', 7000);
		return;
	}
...
function guidesMenu(anchor) {
	...
	showMenu(anchor, [
		...
		checkboxRow('Safe area', g.safeArea.enabled, (v) => (g.safeArea.enabled = v)),
		numberRow('Safe top %', g.safeArea.top, (v) => (g.safeArea.top = v)),
		numberRow('Safe bottom %', g.safeArea.bottom, (v) => (g.safeArea.bottom = v)),
...
function resetMenu(anchor) {
	...
	showMenu(anchor, [
		...
		},
		{
			...
			danger: true,
			onClick: async () => {
				if (await confirmDialog('Reset layout', `Remove every override in the "${profile}" layout?`))
		...
		},
		{
			...
			danger: true,
			onClick: async () => {
				if (await confirmDialog('Revert all', 'Discard every unsaved layout change?')) revertAll();
			},
		},
	]);
}
...
function layoutMenu(anchor) {
	...
	for (const profile of PROFILES) {
		const count = Object.keys(state.overrides.working.profiles[profile] ?? {}).length;
...
function statesMenu(anchor) {
	...
	const gameEvents = state.gameEvents.filter((name) => !name.startsWith('__layoutEditor'));
...
export function initToolbar() {
	...
	$('btn-menu-setup').addEventListener('click', (event) => setupMenu(event.currentTarget));

	// menus
	$('btn-menu-guides').addEventListener('click', (event) => guidesMenu(event.currentTarget));
	$('btn-menu-reset').addEventListener('click', (event) => resetMenu(event.currentTarget));
	$('btn-menu-layout').addEventListener('click', (event) => layoutMenu(event.currentTarget));
	$('btn-menu-states').addEventListener('click', (event) => statesMenu(event.currentTarget));
