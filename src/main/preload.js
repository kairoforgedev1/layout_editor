...
contextBridge.exposeInMainWorld('editorHost', {
	...
	readOverrides: (overridesPath) => ipcRenderer.invoke('overrides:read', { overridesPath }),
	writeOverrides: (overridesPath, data, temporaryContainerIds = []) =>
		ipcRenderer.invoke('overrides:write', { overridesPath, data, temporaryContainerIds }),
	...
	analyzeIntegration: (project) => ipcRenderer.invoke('integration:analyze', { project }),
	installIntegration: (project) => ipcRenderer.invoke('integration:install', { project }),
	rebuildBridge: (workspaceRoot) => ipcRenderer.invoke('integration:rebuild', { workspaceRoot }),
	normalRunCheck: (url) => ipcRenderer.invoke('integration:normalRunCheck', { url }),
	checkWritable: (target) => ipcRenderer.invoke('integration:checkWritable', { target }),
	readDoc: (name) => ipcRenderer.invoke('integration:readDoc', { name }),
	scanAssets: (appDir, sinceMs) => ipcRenderer.invoke('assets:scan', { appDir, sinceMs }),
	registerAssets: (appDir, entries) => ipcRenderer.invoke('assets:register', { appDir, entries }),
