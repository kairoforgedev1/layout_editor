...
const { startProc, restartProc, stopProc, stopAll, probePort } = require('./processes');
...
const configPath = () => path.join(app.getPath('userData'), 'layout-editor-config.json');
...
function createWindow() {
	...
	if (process.env.SLE_DEBUG === '1') win.webContents.openDevTools({ mode: 'detach' });
}
...
ipcMain.handle('overrides:read', (_event, { overridesPath }) => readOverrides(overridesPath));

ipcMain.handle('overrides:write', (_event, { overridesPath, data, temporaryContainerIds }) => {
	try {
...
ipcMain.handle('proc:start', (_event, { kind, cwd, command, args, expectedPort }) =>
...
ipcMain.handle('testcases:scan', (_event, { appDir }) => {
	try {
		...
		return { ok: false, error: String(error.message ?? error), manifests: [], fileErrors: [] };
	}
});
...
// ---------------------------------------------------------------------------
ipcMain.handle('integration:analyze', (_event, { project }) => analyzeIntegration(project));

ipcMain.handle('integration:install', (_event, { project }) => installIntegration(project));
	...
	return new Promise((resolve) => {
		...
		const onChunk = (chunk) => {
			...
			for (const line of chunk.toString().split(/\r?\n/)) {
				if (line.trim()) sendProcEvent({ kind: 'dev', event: 'log', line: `[rebuild] ${line.slice(0, 400)}` });
			}
		};
		child.stdout?.on('data', onChunk);
		child.stderr?.on('data', onChunk);
		child.on('exit', (code) => resolve({ ok: code === 0, code, output: output.slice(-2000) }));
...
ipcMain.handle('integration:normalRunCheck', async (_event, { url, timeoutMs = 25000 }) => {
	let hidden = null;
	try {
...
ipcMain.handle('integration:readDoc', (_event, { name }) => {
	...
	try {
		return { ok: true, path: path.join(DOCS_DIR, file), text: fs.readFileSync(path.join(DOCS_DIR, file), 'utf8') };
	} catch (error) {
		return { ok: false, error: String(error.message ?? error) };
	}
});
...
ipcMain.handle('debug:capture', async (_event, { outPath } = {}) => {
	...
	const target = outPath || path.join(app.getPath('temp'), `sle-capture-${Date.now()}.png`);
...
if (process.env.SLE_DEBUG === '1') {
	...
	const server = http.createServer((req, res) => {
		...
		req.on('end', async () => {
			...
			try {
				const { action, code, frameUrlIncludes, outPath } = JSON.parse(body || '{}');
				if (action === 'eval') {
					...
					const image = await win.webContents.capturePage();
					const target = outPath || path.join(app.getPath('temp'), `sle-${Date.now()}.png`);
					fs.writeFileSync(target, image.toPNG());
