...
const findMockRgs = (workspaceRoot) => {
	...
	for (const candidate of candidates) {
		...
		if (pkg && (pkg.name === 'mock-rgs-server' || exists(path.join(candidate, 'src', 'index.js')))) {
			return candidate;
		}
	}
	return null;
...
function inspectProject(pickedDir) {
	...
	if (!appDir) {
		return { ok: false, error: 'No Stake Engine game app (with a pixi-svelte dependency and src/routes) found in the selected folder.' };
	}
	const workspaceRoot = findWorkspaceRoot(appDir);
	const pkg = readJson(path.join(appDir, 'package.json'));
	const env = parseEnvFile(path.join(appDir, '.env.local'));
	const overridesPath = path.join(appDir, 'src', 'game', 'layoutOverrides.data.ts');
	const pixiSveltePatched = workspaceRoot
		? exists(path.join(workspaceRoot, 'packages', 'pixi-svelte', 'dist', 'layoutOverrides.svelte.js'))
		: false;
	const loaderWired = exists(path.join(appDir, 'src', 'game', 'layoutOverrides.ts'));
	...
	return {
		...
		env: {
			...
			rgsUrl: env.VITE_RGS_URL ?? 'http://localhost:3002',
