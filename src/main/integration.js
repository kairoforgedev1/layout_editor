...
const EXPECTED_BRIDGE_REVISION = '2026-08-21-spawned-runtime-hooks-v1';
...
const PIXI_SVELTE_NAMED_IMPORT_RE = /import\s*\{([\s\S]*?)\}\s*from\s*(['"])pixi-svelte\2\s*;?/g;
const GAME_LAYOUT_LINE_RE = /^([ \t]*)registerGameLayout\s*\(\s*\(\s*\)\s*=>\s*stateLayoutDerived\s*\.\s*mainLayout\s*\(\s*\)\s*\)\s*;?[ \t]*(?=\r?$)/m;
const LEGACY_GAME_SCALE_LINE_RE = /^([ \t]*)registerGameScale\s*\(\s*\(\s*\)\s*=>\s*stateLayoutDerived\s*\.\s*mainLayout\s*\(\s*\)\s*\.\s*scale\s*\)\s*;?[ \t]*(?=\r?$)/m;
...
const inspectGameLayoutLoader = (source) => {
	...
	if (pixiSvelteImportCount(source, 'registerGameLayout') === 1 && GAME_LAYOUT_LINE_RE.test(source)) {
		return { status: 'ok', source };
	}

	const legacyLines = source.match(new RegExp(LEGACY_GAME_SCALE_LINE_RE.source, 'gm')) ?? [];
	if (legacyLines.length !== 1 || pixiSvelteImportCount(source, 'registerGameScale') !== 1) {
		return { status: 'manual', source };
	}

	let importReplacements = 0;
	let upgraded = source.replace(PIXI_SVELTE_NAMED_IMPORT_RE, (full, bindings) => {
		if (!/\bregisterGameScale\b/.test(bindings)) return full;
		importReplacements += 1;
		return full.replace(bindings, bindings.replace(/\bregisterGameScale\b/, 'registerGameLayout'));
	});
	if (importReplacements !== 1) return { status: 'manual', source };

	upgraded = upgraded.replace(LEGACY_GAME_SCALE_LINE_RE, (_line, indent) =>
...
const PATCHES = [
	{
		id: 'utils-hook',
		title: 'propsSyncEffect applies overrides (pixi-svelte/src/lib/utils.svelte.ts)',
		file: (ctx) => path.join(ctx.pixiSvelteLib, 'utils.svelte.ts'),
		applied: (source) => source.includes('applyLayoutOverrides(targetInstance, assignedKeys as string[])'),
		...
		imports: [`import { applyLayoutOverrides } from './layoutOverrides.svelte';`],
	},
	{
		id: 'context-hook',
		title: 'addToParent registers nodes (pixi-svelte/src/lib/context.svelte.ts)',
	...
	},
	{
		...
		file: (ctx) => path.join(ctx.pixiSvelteLib, 'components', 'InitialiseApplication.svelte'),
...
<BaseSprite label={key} {...baseSpriteProps} {texture} />`,
	...
	{
		id: 'spine-label',
		title: 'Spine label defaults to asset key (SpineProvider.svelte) — optional',
		optional: true,
		file: (ctx) => path.join(ctx.pixiSvelteLib, 'components', 'SpineProvider.svelte'),
		applied: (source) => /label=\{key\}/.test(source),
...
const OWNED_FILES = [
	{ id: 'runtime', template: 'layoutOverrides.svelte.ts', target: (ctx) => path.join(ctx.pixiSvelteLib, 'layoutOverrides.svelte.ts') },
	{ id: 'spawned', template: 'spawnedElements.svelte.ts', target: (ctx) => path.join(ctx.pixiSvelteLib, 'spawnedElements.svelte.ts') },
	{ id: 'performance', template: 'performanceSampler.ts', target: (ctx) => path.join(ctx.pixiSvelteLib, 'performanceSampler.ts') },
	{ id: 'test-book-request', template: 'testBookRequest.ts', target: (ctx) => path.join(ctx.pixiSvelteLib, 'testBookRequest.ts') },
	{ id: 'bridge', template: 'editorBridge.ts', target: (ctx) => path.join(ctx.pixiSvelteLib, 'editorBridge.ts') },
];
...
const makeContext = (project) => {
	...
	const pixiSvelteLib = path.join(workspaceRoot, 'packages', 'pixi-svelte', 'src', 'lib');
...
function analyzeIntegration(project) {
	const checks = [];
	const add = (id, title, status, note = '') => checks.push({ id, title, status, note });

	const ctx = makeContext(project);
	if (!ctx) {
		add('structure', 'Standard web-sdk structure (packages/pixi-svelte/src/lib)', 'manual',
			'pixi-svelte source not found — this project needs a manual/AI-assisted setup.');
		return { status: 'manual', expectedVersion: EXPECTED_BRIDGE_VERSION, checks };
	}
	add('structure', 'Standard web-sdk structure', 'ok');

	// editor-owned files
	const runtimeSource = read(path.join(ctx.pixiSvelteLib, 'layoutOverrides.svelte.ts'));
	const expectedRuntimeSource = read(path.join(TEMPLATES_DIR, 'layoutOverrides.svelte.ts'));
	const version = detectVersion(runtimeSource);
	if (!runtimeSource) add('runtime', 'Override runtime (layoutOverrides.svelte.ts)', 'installable', 'missing');
	else if (normalizeSource(runtimeSource) !== normalizeSource(expectedRuntimeSource))
		add('runtime', 'Override runtime (layoutOverrides.svelte.ts)', 'outdated',
			`installed runtime does not match bundled v${EXPECTED_BRIDGE_VERSION} (${EXPECTED_BRIDGE_REVISION})`);
	else add('runtime', 'Override runtime (layoutOverrides.svelte.ts)', 'ok', `v${version} · ${EXPECTED_BRIDGE_REVISION}`);
	...
	for (const [id, file, title] of [
		...
		const status = installed == null
			? 'installable'
			: expected != null && normalizeSource(installed) === normalizeSource(expected)
				? 'ok'
				: 'outdated';
		add(
			id,
	...
	const exportsOk =
		indexSource.includes('./layoutOverrides.svelte') && indexSource.includes('./spawnedElements.svelte');
	add('exports', 'Package exports (index.ts)', exportsOk ? 'ok' : 'installable', exportsOk ? '' : 'missing export lines');
	...
	for (const patch of PATCHES) {
		...
		if (!source) {
			add(patch.id, patch.title, patch.optional ? 'skip' : 'manual', 'file not found');
			continue;
		}
		if (patch.applied(source)) {
			add(patch.id, patch.title, 'ok');
		} else if ((patch.findAlternatives ?? [patch.find]).some((snippet) => flexibleRegex(snippet).test(source)) &&
			(!patch.secondFind || flexibleRegex(patch.secondFind).test(source))) {
			add(patch.id, patch.title, 'installable', 'anchor matched — can patch automatically');
		} else {
			add(patch.id, patch.title, patch.optional ? 'skip' : 'manual',
	...
	if (!stateLayoutOk || !stateAppOk) {
		add('app-shape', 'App exposes src/game/stateLayout.ts + stateApp.ts', 'manual',
	...
	add('app-loader', 'App loader (src/game/layoutOverrides.ts)',
		fs.existsSync(loaderPath) ? 'ok' : stateLayoutOk && stateAppOk ? 'installable' : 'manual');
	...
	add('app-data', 'Layout data file (src/game/layoutOverrides.data.ts)',
		fs.existsSync(path.join(gameDir, 'layoutOverrides.data.ts')) ? 'ok' : 'installable');
	const contextSource = read(path.join(gameDir, 'context.ts'));
	add('app-import', 'Loader imported from src/game/context.ts',
		contextSource == null ? 'manual' : contextSource.includes('./layoutOverrides') ? 'ok' : 'installable',
		contextSource == null ? 'context.ts not found' : '');

	// built dist present?
	const distSource = read(path.join(ctx.workspaceRoot, 'packages', 'pixi-svelte', 'dist', 'layoutOverrides.svelte.js'));
	const distVersion = detectVersion(distSource);
	const distRevision = detectRevision(distSource);
	const distStatus = !distSource
		? 'installable'
		: distVersion === EXPECTED_BRIDGE_VERSION && distRevision === EXPECTED_BRIDGE_REVISION
			? 'ok'
			: 'outdated';
	add('dist', 'pixi-svelte built with the bridge (dist/)', distStatus,
		distStatus === 'ok' ? `v${distVersion} · ${distRevision}` : 'needs `pnpm --filter pixi-svelte build`');
...
function installIntegration(project) {
	...
	const ctx = makeContext(project);
	if (!ctx) return { ok: false, error: 'Project structure not compatible with automatic setup.', results };
	if (!fs.existsSync(TEMPLATES_DIR)) {
		return { ok: false, error: `Bundled bridge templates not found at ${TEMPLATES_DIR}`, results };
	}
	...
	try {
		...
		for (const owned of OWNED_FILES) {
			...
			if (!template) return { ok: false, error: `template ${owned.template} missing`, results };
			if (read(target) === template) {
				add(owned.id, 'unchanged');
			} else {
				writePatched(target, template);
				add(owned.id, fs.existsSync(`${target}.sle-backup`) ? 'updated' : 'created');
			}
		}
		...
		for (const patch of PATCHES) {
			...
			const matchedFind = (patch.findAlternatives ?? [patch.find])
				.find((snippet) => flexibleRegex(snippet).test(source));
			const findRe = matchedFind ? flexibleRegex(matchedFind) : null;
			if (!findRe || (patch.secondFind && !flexibleRegex(patch.secondFind).test(source))) {
				add(patch.id, patch.optional ? 'skipped' : 'manual', 'anchor not found — apply by hand (see setup guide)');
				continue;
			}
			source = source.replace(findRe, () => patch.replace.trim());
			if (patch.secondFind)
				source = source.replace(flexibleRegex(patch.secondFind), () => patch.secondReplace.trim());
			for (const importLine of patch.imports) source = insertAfterLastImport(source, importLine);
		...
		if (!fs.existsSync(dataPath)) {
			const dataTemplate = read(path.join(TEMPLATES_DIR, 'app', 'layoutOverrides.data.ts'));
		...
		if (!fs.existsSync(loaderPath)) {
			if (fs.existsSync(path.join(gameDir, 'stateLayout.ts')) && fs.existsSync(path.join(gameDir, 'stateApp.ts'))) {
				const loaderTemplate = read(path.join(TEMPLATES_DIR, 'app', 'layoutOverrides.ts'));
				fs.writeFileSync(loaderPath, loaderTemplate, 'utf8');
				add('app-loader', 'created');
				add('game-layout', 'created');
			} else {
				add('app-loader', 'manual', 'stateLayout.ts / stateApp.ts not found — write the loader by hand');
				add('game-layout', 'manual', 'register the game content frame in the app loader');
			}
		} else {
			...
			if (gameLayout.status === 'upgradeable') {
				...
				add('app-loader', 'patched', 'preserved existing game-specific loader hooks');
				add('game-layout', 'patched', 'registerGameScale upgraded to registerGameLayout');
			} else {
		...
		if (contextSource == null) {
			add('app-import', 'manual', 'src/game/context.ts not found — import ./layoutOverrides from your app entry');
